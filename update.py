...
class UpdateWindow:
    ...
    def create_main_label(self):
        ...
        self.main_label.pack(pady=(1/30)*self.width)
    ...
    def searchByFolderID(self):
        ...
        def create_update_function(person, window):
            def update_person():
                person.update(window)
        ...
        for i in range(len(data)):
            ...
            newResultButton = tk.Button(self.buttonList_label, text=string, font=('Arial', int(self.font_size//1.6)), command=create_update_function(person, self.window))
    ...
    def searchBySurname(self):
        ...
        for i in range(len(data)):
            ...
            # Creating the result button and adding it to the Result Button List
            newResultButton = tk.Button(self.buttonList_label, anchor="w", text=string, font=('Arial', int(self.font_size//1.6)), command=create_update_function(person, self.window))
            newResultButton.pack(side="top", fill='x')
            newResultButton.bind('<MouseWheel>', self.on_mousewheel)
