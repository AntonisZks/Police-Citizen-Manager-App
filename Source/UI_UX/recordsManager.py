...
def makeRecordButtonText(index: int, record: Record):
    ...
    if record.father_name == "":
        ...
        if ending == "ΗΣ":
            button_text += f"{record.surname} {record.name}   του {record.father_name[:-1]}"
        elif ending == "ΑΣ":
            button_text += f"{record.surname} {record.name}   του {record.father_name[:-1]}"
        elif ending == "ΟΣ":
            button_text += f"{record.surname} {record.name}   του {record.father_name[:-1]}Υ"
        elif ending in ("ης", "ησ"):
            button_text += f"{record.surname} {record.name}   του {record.father_name[:-1]}"
        elif ending in ("ας", "ασ"):
            button_text += f"{record.surname} {record.name}   του {record.father_name[:-1]}"
        elif ending in ("ος", "οσ"):
            button_text += f"{record.surname} {record.name}   του {record.father_name[:-1]}υ"
        else:
            button_text += f"{record.surname} {record.name}   του {record.father_name}"
...
class RecordsManager:
    ...
    def __onButtonClick(self, records):
        selected_indices = [i for i, value in enumerate(self.checkButtonsVar) if value.get() == 1]
    ...
    def createRecordButtons(self, records_df, font):
        ...
        self.checkButtonsVar = [tk.IntVar() for _ in range(len(records))]
        index: int = 0
        for i, record in enumerate(records):
            new_record_button = tk.Checkbutton(
                ...
                variable=self.checkButtonsVar[i - 1]
            )
        ...
        if index > 1:
            self.notebook.tab(0, text=f"{index} αποτελέσματα")
        else:
            self.notebook.tab(0, text=f"{index} αποτέλεσμα")
