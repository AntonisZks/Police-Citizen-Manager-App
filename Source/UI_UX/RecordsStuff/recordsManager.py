...
class RecordsManager:
    ...
    def __onButtonClick(self, records: list[Record]) -> None:
        ...
        for index in selected_indices:
            ...
            if index not in self.selected_buttons:
                self.visualiser.createTab(index, records[index + 1])  # if the current index is not in the current selected button indexes, then create a new tab
        ...
        for index in list(self.selected_buttons.keys()):
            ...
            if index not in selected_indices:
                self.visualiser.removeTab(-1) if solve else self.visualiser.removeTab(index)  # If the current index is not in the current selected buttons indexes, then remove its tab
    ...
    def createRecordButtons(self, records_df: pd.DataFrame, font: tuple[str, int]) -> None:
        ...
        records = []  # Creating an empty list that will contain all the records

        # Iterate through the records dataframe and add every line to the records list
        for _, record in records_df.iterrows():
            records.append(Record(*record.to_list()))  # Convert every line of the dataframe to a python list and add it to the records list
        ...
        self.checkButtonsVar = [tk.IntVar() for _ in range(len(records))]  # Creating a list containing the states of every button (ON/OFF)
        index: int = 0
    ...
    def createNoRecordsMessage(self) -> None:
        ...
        self.no_records_message.pack(pady=round(0.3 * self.app_data['window-height']))  # Packing the label onto the screen
